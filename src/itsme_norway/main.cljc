(ns itsme-norway.main
  "Kotodama WASM entrypoint for the Itsme_norway clean-room actor (L4) — Clojure port.
  ...
  facts (`itsme_norway.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "itsme_norway")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"itsme_norway.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "itsme_norway-compat" :tier tier :entities entities} 200])
...
(defn start! [] :itsme_norway-compat/ready)
